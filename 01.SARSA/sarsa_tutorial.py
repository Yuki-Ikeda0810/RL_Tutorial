...
agent = sarsa_agent.SarsaAgent(env.observation_space.shape[0], env.action_space.n, num_discretize)
...
for episode in range(num_episode):
    ...
    for t in range(max_steps):
        ...
        next_action = agent.get_action(next_state, episode)                 # 次の行動を選択．
        agent.update_qtable(state, action, reward, next_state, next_action) # Q値の表を更新．
        state, action = next_state, next_action                             # 状態と行動を更新
